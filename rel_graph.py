...
def build_deployment_node_graph(plan_node_graph,
    ...
    # ADDED: construct removed_ids_include_hint list, so
    # related instances are removed, not random ones
    _build_removed_ids_include_hint(ctx)
...
def _build_removed_ids_include_hint(ctx):
    if not ctx.modified_nodes:
        return

    node_list = list(ctx.modified_nodes.keys())

    for node in node_list:
        previous_node_instance_ids = ctx.get_node_instance_ids_by_node_id(node)
        for instance_id in previous_node_instance_ids:
            for instance in extract_node_instances(ctx.previous_deployment_node_graph):
                if instance.id == instance_id and instance.relationships:
                    for relationship in instance.relationships:
                        if relationship['target_name'] in node_list:
                            removed_ids_include_hint_for_source_instance = ctx.modified_nodes[instance['name']] \
                                                       .get('removed_ids_include_hint', [])
                            if not removed_ids_include_hint_for_source_instance:
                                ctx.modified_nodes.get(instance['name'])['removed_ids_include_hint'] = []
                            if instance_id not in removed_ids_include_hint_for_source_instance:
                               ctx.modified_nodes[instance['name']]['removed_ids_include_hint'].append(instance_id)

                            removed_ids_include_hint_for_target_instance = ctx.modified_nodes[relationship['target_name']] \
                                                       .get('removed_ids_include_hint', [])
                            if not removed_ids_include_hint_for_target_instance:
...
def _handle_connected_to_and_depends_on(ctx):
    ...
    if ctx.modification:
        previous_target_node_instance_ids = set()
        previous_source_node_instance_ids = set()
        for s, t, e_data in ctx.previous_deployment_node_graph.edges_iter(
            ...
            previous_target_node_instance_ids.add(t_node['id'])
            previous_source_node_instance_ids.add(s_node['id'])
    ...
    for source_node_id, target_node_id, edge_data in connected_graph.edges(
        ...
        target_node_instance_ids = ctx.get_node_instance_ids_by_node_id(
            target_node_id)
        source_node_instance_ids = ctx.get_node_instance_ids_by_node_id(
            source_node_id)
        ...
        existing_target_node_instance_ids = []
        existing_source_node_instance_ids = []
...
def _find_one_to_one_relationship(ctx, relationship_instance):
    for instance in extract_node_instances(ctx.previous_deployment_node_graph):
        if instance.relationships:
            for relationship in instance.relationships:
                if relationship_instance['target_id'] == relationship['target_id']:
                    return None
